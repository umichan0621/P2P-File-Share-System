...
namespace file
{
	...
	public://MD5验证相关
		virtual void check_segment_md5(uint64_t SegmentStart, MD5& MD5Obj) = 0;//校验2M的Segment完整性
		virtual bool md5_segment(MD5& MD5Obj,uint64_t SegmentStart) = 0;//计算文件部分内容的MD5
	public://接受消息之后下载任务完成
	...
	public://文件信息
		...
		void set_sha1(const SHA1& SHA1Struct);
		uint32_t file_seq();
		uint64_t full_size();
		void full_sha1(SHA1& SHA1Struct);		//获取当前文件设定的正确MD5值
		bool sha1(SHA1& SHA1Struct);			//计算文件当前的SHA1
