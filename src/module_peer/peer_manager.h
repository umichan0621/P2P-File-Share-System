...
//Session:与Peer一样，但是相互之间有可靠的UDP连接，更适合推荐给其他节点，而且更有可能是因为互相有感兴趣的文件而建立连接
//当前类用于管理所有的Peer和Session，其中Session是Peer的子集
class PeerManager
{
private:
	struct Peer
...
public:
	...
	//返回Peer的引用
	const sockaddr* peer(int32_t PeerId, PeerStatus& Status);
...
#define g_pPeerManager base::Singleton<PeerManager>::get_instance()