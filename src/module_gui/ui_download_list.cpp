...
namespace gui
{
    DownloadList::DownloadList(QWidget* Parent)
        :QWidget(Parent), m_CurChoose(STATUS_DOWNLOAD)
    {
        //初始化组件
    ...
    void DownloadList::set_style(const QString& Style, const QString& Language)
    {
        ...
        pScrollBar->setProperty("Style", m_strStyle);
    ...
    void DownloadList::set_button_style()
    {
        ...
        {
            m_pPause->setProperty("Button", "unchoose_"+m_strStyle);
            m_pComplete->setProperty("Button", "unchoose_" + m_strStyle);
            m_pDownloading->setProperty("Button", "choose_" + m_strStyle);
        }
        else if (STATUS_PAUSE == m_CurChoose)
        {
            m_pDownloading->setProperty("Button", "unchoose_" + m_strStyle);
            m_pComplete->setProperty("Button", "unchoose_" + m_strStyle);
            m_pPause->setProperty("Button", "choose_" + m_strStyle);
        }
        else if (STATUS_COMPLETE == m_CurChoose)
        {
            m_pDownloading->setProperty("Button", "unchoose_" + m_strStyle);
            m_pPause->setProperty("Button", "unchoose_" + m_strStyle);
            m_pComplete->setProperty("Button", "choose_" + m_strStyle);
        }
        style()->unpolish(m_pDownloading);
        style()->polish(m_pDownloading);
        style()->unpolish(m_pPause);
        style()->polish(m_pPause);
        style()->unpolish(m_pComplete);
        style()->polish(m_pComplete);
    }
